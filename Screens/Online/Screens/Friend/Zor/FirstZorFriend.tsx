...
export default function FirstZorFriend() {
  ...
  const cleanupMatch = async () => {
    ...
    try {
      const matchRef = ref(db, `friendMatchmaking/zor/${matchId}`);
  ...
  useEffect(() => {
    const matchesRef = ref(db, "friendMatchmaking/zor");
  ...
  useEffect(() => {
    ...
    return onValue(matchRef, async (snap) => {
      ...
      if (data.status === "accepted" && allReady) {
        ...
        await remove(ref(db, `friendMatchmaking/zor/${matchId}`));

        navigation.replace("ZorFriendGame", {
          roomId: matchId,
          boardSize: "4x6",
        });
      }
  ...
  const acceptInvite = async () => {
    ...
    await update(ref(db, `friendMatchmaking/zor/${matchId}`), {
      status: "accepted",
    });
  };
  ...
  screen: "FirstFriend",
          ...
          <Text style={styles.subLogo}>HARD FRIEND BATTLE</Text>
        </View>
        ...
        <View style={styles.heroCard}>
          ...
          <Text style={styles.title}>{t.hardFriendGame}</Text>
          <Text style={styles.subtitle}>{t.hardFriendSubtitle}</Text>

          <View style={styles.infoRow}>
            <View style={styles.infoBox}>
              <Icon name="grid" size={22} color="#00D2FF" />
              <Text style={styles.infoLabel}>Tahta</Text>
              <Text style={styles.infoValue}>4x6</Text>
            </View>
