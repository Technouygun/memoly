...
export default function FirstBasitFriend() {
  ...
  const cleanupMatch = async () => {
    ...
    try {
      const matchRef = ref(db, `friendMatchmaking/basit/${matchId}`);
  ...
  useEffect(() => {
    const matchesRef = ref(db, "friendMatchmaking/basit");
  ...
  useEffect(() => {
    ...
    return onValue(matchRef, async (snap) => {
      ...
      if (data.status === "accepted" && allReady) {
        ...
        await remove(ref(db, `friendMatchmaking/basit/${matchId}`));

        navigation.replace("BasitFriendGame", {
          roomId: matchId,
          boardSize: "4x4",
        });
      }
  ...
  const handleInvite = async (friendUid: string) => {
    ...
    const matchRef = ref(db, `friendMatchmaking/basit/${id}`);
  ...
  const acceptInvite = async () => {
    ...
    await update(ref(db, `friendMatchmaking/basit/${matchId}`), {
      status: "accepted",
    });
  };
  ...
  const handleReady = async () => {
    ...
    await update(
      ref(db, `friendMatchmaking/basit/${matchId}/players/${user.uid}`),
      { ready: true }
    );
  };
  ...
  screen: "FirstFriend",
          ...
          <Text style={styles.subLogo}>EASY FRIEND BATTLE</Text>
        </View>
        ...
        <View style={styles.heroCard}>
          ...
          <Text style={styles.title}>{t.easyFriendGame}</Text>
          <Text style={styles.subtitle}>{t.easyFriendSubtitle}</Text>

          <View style={styles.infoRow}>
            <View style={styles.infoBox}>
              <Icon name="grid" size={22} color="#00D2FF" />
              <Text style={styles.infoLabel}>Tahta</Text>
              <Text style={styles.infoValue}>4x4</Text>
            </View>
