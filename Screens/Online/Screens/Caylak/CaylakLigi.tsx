...
export default function CaylakLigi() {
  ...
  const checkCoin = async () => {
    ...
    if (coins < ENTRY_FEE) {
      Alert.alert(
        "Yetersiz Coin",
        `Çaylak ligine girmek için en az ${ENTRY_FEE} Coin gerekli.`
      );
      return false;
    }
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      if (waitingSnap.exists()) {
        ...
        const id = `caylak_${opponent.uid.slice(0, 6)}_${user.uid.slice(
          0,
          6
        )}_${Date.now()}`;
  ...
  const handleReady = async () => {
    ...
    await update(
      ref(db, `caylakMatchmaking/matches/${matchId}/players/${user.uid}`),
      { ready: true }
    );
